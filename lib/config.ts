...
export const config = {
     ...
     * row (or archive) can't balloon a batch — a memory/DoS bound, NOT an API
     * one: images are downscaled server-side (imageDownscale.ts) before the
     * model call, so the API's 10 MB-per-image limit no longer constrains
     * intake. */
