...
- **Uploaded images are sent to the model unmodified — no normalization step.** Raw image bytes are base64-encoded and passed straight through (no image library is installed); only PDFs are touched, and only to slice pages. We deliberately skip resize/re-encode because the Anthropic API already downsizes to its own token cap, so normalization would buy little *speed* — that latency is fixed per call regardless of image size. The robustness gaps it leaves are EXIF orientation (a sideways phone photo is sent rotated) and the API's hard per-image size cap (an oversized image is rejected, not scaled). Neither affects the PDF path; both are live only for the direct-image and CSV-image paths. A normalization pass (bake EXIF orientation + cap the long edge) at the single point where all images converge (`buildSourceBlock`) is the obvious hardening if testing on real phone-camera/scanner uploads shows misreads — held off here as unnecessary for the curated samples, added only if the data calls for it.
