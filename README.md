...
- **Tolerant fields apply field-aware normalization.** Before scoring, the producer name/address folds away a label-only "BOTTLED BY"-style prefix and maps full US state names to abbreviations ("…Charleston, South Carolina" ≡ "…, SC"), and the fanciful name drops a leading vintage year ("2023 Rosé" ≡ "Rosé"). This is applied to both sides and only affects the *scored* text (displayed values stay verbatim). The trade-off is a small, rare false-pass surface — e.g. a company literally named after a state, or two fanciful names differing only by a leading year — bounded by the confidence gate and human review; it can't affect the government warning.
